...
type CreateOfferResult = { success: false; error: string };

export async function createOffer(input: OfferInput): Promise<CreateOfferResult> {
  ...
  // Re-check overlap server-side (the client already blocks it): load this
  // product's existing rules in the org and reject a conflicting range.
  ...
  if (overlapsExistingRule(existing ?? [], input.product_id, input.min_qty, input.max_qty)) {
    return {
      success: false,
  ...
  const { error } = await supabase.from("quantity_price_rules").insert({
